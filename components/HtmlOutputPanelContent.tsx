...
const SkeletonLoader: React.FC = () => (
  <div className="w-full p-4">
    {[1, 2, 3, 4, 5, 6, 7].map((_, i) => (
      <div
        key={i}
        className="h-3 mb-2 bg-zinc-800 rounded animate-pulse"
        style={{
  ...
  if (html) {
    return (
      <div className="relative w-full">
        ...
        <div className="px-4 py-1.5 border-b border-zinc-800 bg-zinc-900/50 font-mono text-[10px] text-zinc-500 tracking-widest flex justify-between">
          <span>// HTML_OUTPUT</span>
          <span>{html.length.toLocaleString()} CHARS</span>
        </div>
        <pre className="m-0 p-4 font-mono text-sm leading-relaxed text-zinc-300 bg-transparent whitespace-pre-wrap break-all tracking-wide max-h-[500px] overflow-y-auto">
